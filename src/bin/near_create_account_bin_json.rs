...
// `near` on mainnet, `testnet` on testnet) using raw JSON args.
// The TLD contract id is resolved automatically from `NEAR_NETWORK`.
//
// set NEAR_NETWORK, NEAR_ACCOUNT_ID, NEAR_PRIVATE_KEY in env (.env)
// then run:
//   cargo run --bin near_create_account_bin_json -- <new_account_id> <new_public_key>
//
// example:
//   cargo run --bin near_create_account_bin_json -- newaccounttests.testnet ed25519:56CJx1YrTJ6nBmJb8DoXwBm5HLHZcoyAt8FQDmcvkjFE
//
// =================================================
use near_kit::Error;
...
async fn main() -> Result<(), Error> {
    ...
    let new_account_id = args
        .get(1)
        .expect("usage: near_create_account_bin_json <new_account_id> <new_public_key>");
